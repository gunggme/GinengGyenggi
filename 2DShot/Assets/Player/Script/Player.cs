using System.Collections;
using System.Collections.Generic;
using UnityEngine;

public class Player : MonoBehaviour
{
    [Header("PlayerStat")]
    [SerializeField] float speed;
    [SerializeField] public int power;
    [SerializeField] public float fuer;
    [SerializeField] public int hp;

    [Header("ShotDelay")]
    [SerializeField] float curDelay;
    [SerializeField] float maxDelay = 0.3f;

    [Header("ShotPosition")]
    [SerializeField] Transform[] firePosition;

    [Header("Sprite")]
    [SerializeField] SpriteRenderer spri;
    [SerializeField] Sprite[] sprite;

    public bool isDown;

    Coroutine coru;
    bool isMZ;

    bool isHit;

    private void Awake()
    {
        InvokeRepeating("FuerDown", 1, 1);
    }

    private void Update()
    {
        if(hp > 5)
        {
            hp = 5;
            GameManager.instance.HPSet();
        }

        Move();
        Shot();
    }

    void Move()
    {
        float h = Input.GetAxisRaw("Horizontal");
        transform.position += new Vector3(h, Input.GetAxisRaw("Vertical"), 0) * speed * Time.deltaTime;

        switch (h)
        {
            case 0:
                spri.sprite = sprite[1];
                break;
            case 1:
                spri.sprite = sprite[2];
                break;
            case -1:
                spri.sprite = sprite[0];
                break;
        }

        Vector3 pos = Camera.main.WorldToViewportPoint(transform.position);
        if(pos.x < 0) pos.x = 0;
        if (pos.x > 1) pos.x = 1;
        if(pos.y < 0) pos.y = 0;
        if(pos.y > 1) pos.y = 1;
        transform.position = Camera.main.ViewportToWorldPoint(pos);
    }

    void Shot()
    {
        if(curDelay < maxDelay)
        {
            curDelay += Time.deltaTime;
            return;
        }
        if (!Input.GetKey(KeyCode.Z))
            return;

        switch (power)
        {
            case 1:
                GameObject dirC = GameManager.instance.objMana.MakeObj("PlayerBullet1");
                dirC.transform.position = firePosition[0].position;

                Rigidbody2D rigidC = dirC.GetComponent<Rigidbody2D>();
                rigidC.AddForce(Vector3.up * 20, ForceMode2D.Impulse);
                break;
            case 2:
                GameObject dirL = GameManager.instance.objMana.MakeObj("PlayerBullet1");
                GameObject dirR = GameManager.instance.objMana.MakeObj("PlayerBullet1");
                dirL.transform.position = firePosition[1].position;
                dirR.transform.position = firePosition[2].position;

                Rigidbody2D rigidL = dirL.GetComponent<Rigidbody2D>();
                Rigidbody2D rigidR = dirR.GetComponent<Rigidbody2D>();
                rigidL.AddForce(Vector3.up * 20, ForceMode2D.Impulse);
                rigidR.AddForce(Vector3.up * 20, ForceMode2D.Impulse);
                break;
            case 3:
                GameObject dirC3 = GameManager.instance.objMana.MakeObj("PlayerBullet2");
                dirC3.transform.position = firePosition[0].position;

                Rigidbody2D rigidC3 = dirC3.GetComponent<Rigidbody2D>();
                rigidC3.AddForce(Vector3.up * 20, ForceMode2D.Impulse);
                break;
            case 4:
                GameObject dirL4 = GameManager.instance.objMana.MakeObj("PlayerBullet1");
                GameObject dirC4 = GameManager.instance.objMana.MakeObj("PlayerBullet2");
                GameObject dirR4 = GameManager.instance.objMana.MakeObj("PlayerBullet1");
                dirL4.transform.position = firePosition[1].position;
                dirC4.transform.position = firePosition[0].position;
                dirR4.transform.position = firePosition[2].position;

                Rigidbody2D rigidL4 = dirL4.GetComponent<Rigidbody2D>();
                Rigidbody2D rigidC4 = dirC4.GetComponent<Rigidbody2D>();
                Rigidbody2D rigidR4 = dirR4.GetComponent<Rigidbody2D>();
                rigidL4.AddForce(Vector3.up * 20, ForceMode2D.Impulse);
                rigidC4.AddForce(Vector3.up * 20, ForceMode2D.Impulse);
                rigidR4.AddForce(Vector3.up * 20, ForceMode2D.Impulse);
                break;
        }

        curDelay = 0;
    }

    void FuerDown()
    {
        if (isDown)
        {
            fuer -= 5;
        }
    }

    void OnHit()
    {
        hp--;

        OnHitEffect();
        Invoke("ReturnColor", 1);

        GameManager.instance.HPSet(); 

        if(hp < 1)
        {
            //게임오버 되게 만들기
        }
    }

    void OnHitEffect()
    {
        spri.color = new Color(1, 1, 1, 0.5f);
        gameObject.layer = 8;
        isHit = true;
    }

    void ReturnColor()
    {
        spri.color = new Color(1, 1, 1, 1);
        gameObject.layer = 7;
        isHit = false;
    }

    IEnumerator MZ()
    {
        WaitForSeconds asd = new WaitForSeconds(0.1f);
        isMZ = true;
        OnHitEffect();
        yield return new WaitForSeconds(1);
        spri.color = new Color(1, 1, 1, 0.4f);
        yield return asd;
        spri.color = new Color(1, 1, 1, 1);
        yield return asd;
        spri.color = new Color(1, 1, 1, 0.4f);
        yield return asd;
        spri.color = new Color(1, 1, 1, 1);
        yield return asd;
        spri.color = new Color(1, 1, 1, 0.4f);
        yield return asd;
        isMZ = false;
        ReturnColor();
    }

    private void OnTriggerEnter2D(Collider2D collision)
    {
        if(!isHit)
        {
            if (collision.gameObject.CompareTag("Enem"))
            {
                OnHit();
            }
            if (collision.gameObject.CompareTag("EnemBullet"))
            {
                OnHit();
                collision.gameObject.SetActive(false);
            }
        }

        if (collision.gameObject.CompareTag("Item"))
        {
            Item item = collision.gameObject.GetComponent<Item>();
            switch (item.itemName)
            {
                case "Coin":
                    GameManager.instance.score += 1000;
                    break;
                case "Power":
                    if(power > 4)
                    {
                        power = 4;
                        GameManager.instance.score += 300;
                    }
                    else
                    {
                        power++;
                    }
                    break;
                case "HP":
                    if (hp > 5)
                    {
                        hp = 5;
                        GameManager.instance.score += 300;
                        GameManager.instance.HPSet();
                    }
                    else
                    {
                        hp += 1;
                        GameManager.instance.HPSet();
                    }
                    break;
                case "Fuer":
                    if (fuer > 100)
                    {
                        fuer = 100;
                        GameManager.instance.score += 300;
                    }
                    else
                    {
                        fuer += 20;
                    }
                    break;
                case "MZ":
                    if (!isMZ)
                    {
                        coru = StartCoroutine(MZ());
                    }
                    else if (isMZ)
                    {
                        StopCoroutine(coru);
                        coru = StartCoroutine(MZ());
                    }
                    break;
            }
            collision.gameObject.SetActive(false);
        }
    }
}
